require "topological_inventory/google/operations/worker"

RSpec.describe TopologicalInventory::Google::Operations::Worker do
  describe "#run" do
    ...
    before do
      TopologicalInventory::Google::MessagingClient.class_variable_set(:@@default, nil)
    ...
    it "calls subscribe_topic on the right queue" do
      operations_topic = "platform.topological-inventory.operations-google"
      result = double("result")

      expect(client).to receive(:subscribe_topic)
        .with(hash_including(:service => operations_topic)).and_yield(message)
      expect(TopologicalInventory::Google::Operations::Processor)
    ...
    context ".metrics" do
      it "records successful operation" do
        ...
        allow(TopologicalInventory::Google::Operations::Processor).to receive(:process!).and_return(result)
      ...
      it "records exception" do
        ...
        allow(TopologicalInventory::Google::Operations::Processor).to receive(:process!).and_raise("Test Exception!")
        expect(metrics).to receive(:record_operation).with(operation, :status => result)
